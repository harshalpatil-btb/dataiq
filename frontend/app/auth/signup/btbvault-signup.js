    ...
    <div className="min-h-screen flex items-center justify-center bg-[#f6f5f2] px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-[0_4px_24px_rgba(0,0,0,0.08)] border border-[rgba(17,17,25,0.08)]">
          ...
          <p className="text-[#8a8899] text-sm mb-6">Start closing deals smarter with btbVault.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input required type="text" placeholder="Rahul Kumar"
                  value={form.full_name} onChange={e => update('full_name', e.target.value)}
                  className="w-full px-3 py-2.5 rounded-[9px] border-[1.5px] border-[rgba(17,17,25,0.14)] bg-[#f6f5f2] text-sm outline-none focus:border-[#3d5afe] transition-all" />
              </div>
              <div>
            ...
            <div>
              ...
              <input required type="email" placeholder="rahul@acmecorp.com"
                  ...
                  <div className="flex gap-1 mb-1">
                    {[0,1,2,3].map(i => (
                      <div key={i} className="flex-1 h-1 rounded-full transition-all"
                        style={{ background: i < pwScore ? pwColors[pwScore-1] : '#e8e5df' }} />
                    ))}
                  </div>
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3 rounded-[9px] bg-[#3d5afe] text-white text-sm font-bold mt-2 transition-all hover:bg-[#536dfe] hover:-translate-y-0.5 shadow-[0_2px_10px_rgba(61,90,254,0.25)] disabled:opacity-60">
